...
void DebugDraw::DrawSolidPolygon_noedge(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	for (int32 i = 1; i < vertexCount - 1; ++i)
	{
...
void DebugDraw::DrawString_black(const b2Vec2& pw, const char *string,...)
{
	b2Vec2 ps = g_camera.ConvertWorldToScreen(pw);
	...
	ImGui::Begin("Overlay", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollbar);
	ImGui::SetCursorPos(ps);
	ImGui::TextColoredV(ImColor(0, 0, 190, 255), string, arg);
	ImGui::End();
	va_end(arg);
}
