...
void Test::LaunchBomb()
{
	...
	b2Vec2 v = -5.0f * p;
	LaunchBomb(p+g_camera.m_center, v);
}
...
void Test::LaunchBomb(const b2Vec2& position, const b2Vec2& velocity)
{
	...
	if (RandomFloat(0.0f, 1.0f) < 0.1) circle.m_radius = 10.0f;
	else circle.m_radius = RandomFloat(1.0f, 2.0f);

	b2FixtureDef fd;
	fd.shape = &circle;
	if (RandomFloat(0.0f, 1.0f) < 0.1) fd.density = 200.0f;
	else fd.density = 10.0f;
	fd.restitution = .5f;
